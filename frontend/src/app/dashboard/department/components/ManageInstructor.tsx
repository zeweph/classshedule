...
const ManageInstructor: React.FC = () => {
  ...
  // Statistics
  const activeUsers = users.filter(users => users.status === "Active" && users.department_id === user?.department_id).length;
  const instructorUsers = users.filter(users => users.role === "instructor" && users.department_id===user?.department_id).length;
  const studentUsers = users.filter(users => users.role === "student" && users.department_id===user?.department_id).length;
  ...
  return (
    <Container size="lg" py="xl">
      <Stack gap="lg">
        <Box>
          <Title order={1} mb="xs"> Manage instructor and student in {user?.department_name}</Title>
          ...
          <Paper withBorder p="md" radius="md">
            <Group justify="apart">
              <div>
                ...
                <Text fw={700} size="xl">{users?.filter(users=>users.department_id === user?.department_id).length}</Text>
              </div>
        ...
        {/* Main Users Table - No tabs needed since Add User is built into ViewUsers */}
        <ViewUsers
          users={users}
