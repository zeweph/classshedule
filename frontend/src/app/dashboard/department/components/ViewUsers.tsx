...
const [currentUser, setcurrentUserr] = useState<any>(null);
  ...
  const roleCounts = {
    all: users ?.filter(user =>user?.department_id === currentUser?.department_id).length || 0,
    instructor: users?.filter(user => user?.role === "instructor" && user?.department_id === currentUser?.department_id).length || 0,
    student: users?.filter(user => user?.role === "student" && user?.department_id === currentUser?.department_id).length || 0,
  };

  const statusCounts = {
    all: users?.filter(user => user?.department_id === currentUser?.department_id).length || 0,
    Active: users?.filter(user => user?.status === "Active" && user?.department_id === currentUser?.department_id).length || 0,
    Deactivated: users?.filter(user => user?.status === "Deactivated" && user?.department_id === currentUser?.department_id).length || 0,
  };
          ...
          <Group justify="space-between">
            ...
            <Group>
              <Badge color="blue" variant="filled" size="lg">
                {filteredUsers?.filter(user=>user?.department_id === currentUser?.department_id).length} {filteredUsers?.filter(user=>user?.department_id === currentUser?.department_id).length === 1 ? 'user' : 'users'}
              </Badge>
              ...
              <Menu shadow="md" width={200} position="bottom-end">
                ...
                <Menu.Dropdown>
                  ...
                  <Menu.Item 
                    ...
                    disabled={filteredUsers?.filter(user=>user?.department_id === currentUser?.department_id).length === 0}
                  >
                    Export to CSV ({filteredUsers?.filter(user=>user?.department_id === currentUser?.department_id).length})
                  </Menu.Item>
                  <Menu.Item 
                    leftSection={<ArrowDownTrayIcon className="h-4 w-4" />}
                  ...
                  <Menu.Item 
                    ...
                    disabled={users.filter(user=>user?.department_id===currentUser?.department_id).length === 0}
                  >
                    Export All to CSV ({users.filter(user=>user?.department_id===currentUser?.department_id).length})
                  </Menu.Item>
                  <Menu.Item 
                    leftSection={<ArrowDownTrayIcon className="h-4 w-4" />}
                    ...
                    disabled={users.filter(user=>user?.department_id===currentUser?.department_id).length === 0}
                  >
                    Export All to JSON ({users.filter(user=>user?.department_id===currentUser?.department_id).length})
                  </Menu.Item>
                </Menu.Dropdown>
              </Menu>
            </Group>
              ...
              <Group>
                ...
                {(filteredUsers.length > 0 || users.length > 0) && (
                  <Text size="sm" c="dimmed">
                    {hasActiveFilters 
                      ? `${filteredUsers?.filter(user=>user?.department_id === currentUser?.department_id).length} of ${users?.filter(user => user?.department_id === currentUser?.department_id).length} users` 
                      : `${users?.filter(user => user?.department_id === currentUser?.department_id).length} total users`
                    }
                  </Text>
                )}
              </Group>
              ...
              <Grid.Col span={{ base: 12, sm: 6, md: 3 }}>
                <Select
                  ...
                  data={[
                    ...
                    { value: "student", label: `Student (${roleCounts.student})` },
                  ]}
        ...
        <Box style={{ overflowX: 'auto' }}>
          <Table.ScrollContainer minWidth={800}>
            <Table verticalSpacing="sm">
              ...
              <Table.Tbody>
                {!users || users?.filter(user=>user?.department_id === currentUser?.department_id).length === 0 ? (
                  <Table.Tr>
                ...
                ) : filteredUsers?.filter(user=>user?.department_id === currentUser?.department_id).length === 0 ? (
                  <Table.Tr>
                    <Table.Td colSpan={8} style={{ textAlign: 'center' }} py="xl">
                      <Stack gap="xs" align="center">
                  ...
                  filteredUsers.map((user) =>user?.department_id === currentUser?.department_id && (
          ...
          <Box p="md" className="bg-gray-50 border-t">
            <Group justify="space-between">
              ...
              <Group gap="xs">
                ...
                {roleFilter !== "all" && (
                  <Badge color="blue" variant="light" size="sm">
                    {roleFilter}: {roleCounts[roleFilter as keyof typeof roleCounts]}
                  </Badge>
                )}
                {statusFilter !== "all" && (
                  <Badge 
